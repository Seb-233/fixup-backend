...
public class GetProperty {
    ...
    public PropertySummary execute(CurrentActor actor, UUID propertyId) {
        ...
        if (property.status() == PropertyStatus.DELETED && !property.isOwnedOrManagedBy(actor.internalUserId())) {
            throw new PropertyNotFoundException(propertyId);
        }
        if (property.status() == PropertyStatus.DRAFT && !property.isOwnedOrManagedBy(actor.internalUserId())) {
